...
use EmissorNfse\Providers\Webiss\V2_02\Request\RecepcionarLoteRpsRequest;
...
// ###############################################
// Exemplo de como gerar uma nota fiscal utilizando o serviço "RecepcionarLoteRps"
...
$request->setCnpjPrestador('0444469000124');
$request->setInscricaoMunicipalPrestador('1986');
$request->setNumeroLote(3);


$rps = new Rps();

$rps->setCnpjPrestador('0444469000124');
...
if(! empty($response->erros)){
    ...
    // Para o serviço RecepcionarLoteRps é retornado o número de protocolo para que você possa consultar a nota fiscal
    // quando a mesma já estiver processada pelo webservice da Prefeitura
    if(! empty($response->protocolo)){
        // Gerou uma NFS-e
        $protocolo = $response->protocolo;
        $dataRecebimento = $response->dataRecebimento;
        $numeroLote = (! empty($response->numeroLote)) ? $response->numeroLote : null;
        // etc ....
    }else{
